...
1. Follow the operator runbook in `AWS_C6A_DEPLOY_PLAN.md` when you are ready to create AWS, DNS, GitHub secrets, and Vercel production env yourself.
...
- **2026-06-12 database schema policy:** Local Supabase dev uses existing `v-sign_schema`, which contains the current dev data. Production must use a clean dedicated schema such as `vsign_prod`; do not use `public` and do not reuse `v-sign_schema` for production. Updated `.env.prod.example`, `v-sign-be/.env.prod.example`, the AWS deploy plan, and the GHCR/Caddy deploy runbook accordingly.
- **2026-06-12 AWS instance decision update:** Switched the selected 24/7 two-month AWS instance from `c6i.large` to `c6a.large`. The deploy target remains x86_64/`linux/amd64`, but `c6a.large` is preferred because it keeps Docker image compatibility while lowering expected compute cost.
